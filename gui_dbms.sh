...
insert_into_table_gui() {
    ...
    # Insert using library function
    local result
    result=$(insert_into_table "$db_name" "$table_name" "${value_array[@]}" 2>&1)
...
select_where_gui() {
    ...
    local result
    result=$(select_where "$db_name" "$table_name" "$where_col_idx" "$where_value" 2>&1)
...
delete_from_table_gui() {
    ...
    local result
    result=$(delete_from_table "$db_name" "$table_name" "$where_col" "$where_value" 2>&1)
...
update_table_gui() {
    ...
    local result
    result=$(update_table "$db_name" "$table_name" "$update_col" "$new_value" "$search_col" "$search_value" 2>&1)
